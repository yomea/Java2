...
//		session.save(c);
	...
	public void testHQL_01() {
		Session session = sf.openSession();
		session.beginTransaction();//HQL面向对象的方式来编程
...
//		q.setParameter(1, 8);
	...
	public void testHQL_09() {
		Session session = sf.openSession();
		session.beginTransaction();
		Query q = session.createQuery("from Topic t where t.category.id=1");//from Topic t where t,caregory.name='c3'比较一下两者的区别
