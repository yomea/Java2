...
public class JdbcDaoImpl implements Idao {
	...
	public List<Student> findStudents2(Student searchModel, int pageNum, int pageSize) throws SQLException {
		...
		StringBuilder sqlCount = new StringBuilder("select count(id) from t_student where 1=1");
		
		if(username != null) {
			sql.append(" and stu_name like ?");
			sqlCount.append(" and stu_name like ?");
			paramterList.add("%" + username + "%");
		}
		if(gender > 0 && gender != 3) {
			sql.append(" and gender=?");
			sqlCount.append(" and gender=?");
		...
		ResultSet rsCount = Dbutil.getRs(sqlCount.toString(), paramterList);
